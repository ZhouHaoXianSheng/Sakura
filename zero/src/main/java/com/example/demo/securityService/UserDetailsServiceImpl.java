...
import com.example.demo.common.CacheConstants;
...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) {
        ...
        SysUser user = userService.getOne(Wrappers.<com.example.demo.model.SysUser>query().lambda().eq(SysUser::getUsername, username));
        R<UserInfo> result = new R<>();
        if (user == null) {
            result = R.failed(String.format("用户信息为空 %s", username));
        }
        result = R.ok(userService.getUserInfo(user));
